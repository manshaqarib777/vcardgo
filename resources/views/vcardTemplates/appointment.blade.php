    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">{{ __('messages.make_appointment') }}</h3>
            ...
            <div class="modal-body">
                ...
                <div class="mb-5">
                    ...
                    {{ Form::text('name', null, ['class' => 'form-control', 'required', 'placeholder' => __('messages.form.enter_name'), 'id' => 'paypalIntUserName']) }}
                </div>
                <div class="mb-5">
                    {{ Form::label('email', __('messages.common.email') . ':', ['class' => 'form-label required ']) }}
                    {{ Form::text('email', null, ['class' => 'form-control', 'required', 'placeholder' => __('messages.form.enter_email'), 'id' => 'paypalIntUserEmail']) }}
                </div>
                <div class="mb-5">
                    {{ Form::label('phone', __('messages.common.phone') . ':', ['class' => 'form-label']) }}
                    {{ Form::text('phone', null, ['class' => 'form-control', 'placeholder' => __('messages.form.enter_phone'), 'id' => 'paypalIntUserPhone']) }}
                </div>
                <div class="mb-5">
                    <div class="d-flex">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" name="message" placeholder="{{ __('messages.form.type_message') }}"
                        id="paypalIntUserMessage" rows="5"></textarea>
                </div>
                @if (isset($appointmentDetail->is_paid) &&
                    $appointmentDetail->is_paid == 1 &&
                    (getUserSettingValue('stripe_enable', $vcard->user->id) || getUserSettingValue('paypal_enable', $vcard->user->id)))
                    <div class="mb-5">
                        {{ Form::label('payment_method', __('messages.common.payment_methods') . ':', ['class' => 'form-label required']) }}
                        {{ Form::select('payment_method', $appointmentDetail->is_paid == 0 ? \App\Models\Appointment::PAYMENT_METHOD : $paymentMethod, null, ['class' => 'form-control form-select form-select-solid select2Selector', 'data-control' => 'select2', 'required', 'id' => 'appointmentPaymentMethod', 'placeholder' => __('messages.common.payment_methods')]) }}
                    </div>
